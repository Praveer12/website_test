...
import { Calendar, Clock, ArrowRight, X, Sparkles, User2 } from 'lucide-react';
    ...
    <section id="blog" className="py-16 md:py-24" style={{ background: 'var(--bg-body)', transition: 'background 0.3s' }}>
      <div className="site-container">
        ...
        >
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 3rem)', fontWeight: 800, color: 'var(--text-heading)', fontFamily: 'var(--font-heading)', textWrap: 'balance' }}>
        ...
        <div ref={gridRef}>
          ...
          <div
            className={`glass-card reveal-up ${gridVisible ? 'is-visible' : ''}`}
            style={{
          ...
          >
            <div style={{ position: 'relative', minHeight: 280, overflow: 'hidden' }}>
              ...
              <div style={{ position: 'absolute', top: 16, left: 16, padding: '6px 14px', borderRadius: 9999, background: 'var(--primary)', color: '#fff', fontSize: 11, fontWeight: 700 }}>{featured.category}</div>
              <div style={{ position: 'absolute', top: 16, right: 16, padding: '6px 14px', borderRadius: 9999, background: 'rgba(0,0,0,0.6)', color: '#fff', fontSize: 11, fontWeight: 700, backdropFilter: 'blur(8px)' }}>Featured</div>
            </div>
            <div style={{ padding: 'clamp(24px, 4vw, 40px)', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 12, color: 'var(--text-muted)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Calendar style={{ width: 14, height: 14, color: 'var(--accent-cyan)' }} /> {featured.date}</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock style={{ width: 14, height: 14, color: 'var(--primary)' }} /> {featured.readTime}</span>
              </div>
              <h3 style={{ fontSize: 'clamp(1.2rem, 2.5vw, 1.6rem)', fontWeight: 700, color: 'var(--text-heading)', fontFamily: 'var(--font-heading)', lineHeight: 1.3 }}>{featured.title}</h3>
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.7 }}>{featured.excerpt}</p>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 16, borderTop: '1px solid var(--border-main)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <User2 style={{ width: 14, height: 14, color: '#fff' }} />
                  </div>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-body)' }}>{featured.author}</span>
                </div>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: 'var(--primary)' }}>
          ...
          {/* Other Posts — Smaller Grid */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 320px), 1fr))', gap: 24 }}>
            {rest.map((post, idx) => (
              <div
                key={post.id}
                className={`glass-card reveal-blur stagger-${idx + 2} ${gridVisible ? 'is-visible' : ''}`}
              ...
              >
                <div style={{ position: 'relative', height: 180, overflow: 'hidden' }}>
                  <img src={post.image} alt={post.title} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s' }}
                    onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'} />
                  <div style={{ position: 'absolute', top: 12, left: 12, padding: '4px 12px', borderRadius: 9999, background: 'var(--primary)', color: '#fff', fontSize: 10, fontWeight: 700 }}>{post.category}</div>
                </div>
                <div style={{ padding: 20, flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 11, color: 'var(--text-muted)' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Calendar style={{ width: 12, height: 12, color: 'var(--accent-cyan)' }} /> {post.date}</span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock style={{ width: 12, height: 12, color: 'var(--primary)' }} /> {post.readTime}</span>
                  </div>
                  <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-heading)', fontFamily: 'var(--font-heading)', lineHeight: 1.4 }}>{post.title}</h3>
                  <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.6, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{post.excerpt}</p>
                </div>
      ...
      {selectedPost && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'var(--bg-modal-overlay)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setSelectedPost(null)}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-main)', borderRadius: 24, maxWidth: 640, width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: 32, position: 'relative', boxShadow: 'var(--shadow-card)', animation: 'fadeIn 0.2s ease-out' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--border-main)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-muted)' }}>
                <User2 style={{ width: 14, height: 14 }} /> By {selectedPost.author}
              </div>
